...
```
thesis/
├── references/                                   # Input: Raw authoritative source documents
│   └── <stem>.pdf
└── references-extracted/                         # Output: Multimodal document cache
    └── <stem>/
        └── hybrid_txt/
...
Downstream systems (such as `REFERENCES.tsv`) link directly to the primary output document via:
`references-extracted/<stem>/hybrid_txt/<stem>.md`.
...
| `-o` | `references-extracted` | Target output directory where `<stem>/hybrid_txt/` will be generated. |
...
The following turnkey loop scans [`references/`](references/), verifies the cache, and sequentially processes any missing documents through the active API daemon:

```bash
#!/usr/bin/env bash
set -euo pipefail

# Ingest all unextracted PDFs in references/ via the local FastAPI daemon
for pdf in references/*.pdf; do
  [ -e "$pdf" ] || continue
  stem=$(basename "$pdf" .pdf)
  target_md="references-extracted/${stem}/hybrid_txt/${stem}.md"
...
* **Ground-Truth Policy:** Treat markdown `<details>` chart tables as qualitative indicators only. For empirical data, statistical baselines, or quantitative metrics in the thesis, consult the original chart figure in `references-extracted/<stem>/hybrid_txt/images/` or the source PDF.
