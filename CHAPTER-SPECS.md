...
**Sources of truth.** Content, variables, and constants are instantiated from `RESEARCH-DESIGN.md` (RD). Section skeleton from `OUTLINE.md` — **binding: all sections are mandatory** (department format, confirmed). Where content within a section conflicts with RD, RD wins (§G2 lists the four deliberate departures: K = 6 LODO, fold-mean inference, confirmatory-only H0/H1 framing, declared-or-rejected instruments). Nothing in this spec may contradict RD. Terminology governance: term usage syncs with `TERMINOLOGY.tsv`, and every citation the spec requires gets a `corpus-extracted/index.json` → `manuscript/bibliography.bib` entry — additions here must propagate to both.
...
| COA Consolidated Report on the Audit of DRRM Funds (2024; already in `corpus-raw/`) | Recurring low-utilization findings (e.g., 70 LGUs flagged in 2024) — frames efficiency as an institutional concern | 1.3, 1.10 | coa.gov.ph |
