...
Received: 14 December 2024 / Accepted: 2 November 2025 / Published online: 18 December 2025

© The Author(s) 2025

## Abstract

Multi-task learning (MTL) is a machine learning method that has witnessed exponential traction due to its ability to solve multiple tasks simultaneously. Joining training-related tasks in MTL improves both predictive performance and data efficiency. The shared knowledge across tasks enables MTL models to outperform traditional single-task models. The effectiveness of MTL depends on several factors, including how tasks are chosen, how relationships between them are structured, and the choice of shared and task-specific model components. In literature, several architectures and strategies for multi-task learning have been successfully applied across different fields such as natural language processing, computer vision, healthcare, and others. However, MTL has some challenges, such as task interference, data availability, optimal task selection, and hyperparameter tuning. Recent research has introduced new techniques to address these issues, such as adaptive task balancing and parameter sharing mechanisms, although each approach has its limitations. In this review, we provide a comprehensive examination of the multi-task learning concept, and the strategies used in several different domains. It also explores the key factors influencing th success of MTL and provides a detailed classification of research efforts applying MTL in a wide range of fields, highlighting existing achievements and challenges.
...
Unlike multi-task learning, which focuses on training a single model to handle multiple tasks at once, for example, it can involve learning both hate speech and sentiment analysis together or developing two topic classifiers simultaneously.
...
To this end, the contributions of this paper are outlined as follows: First, we offer quantitative insights into the effectiveness of MTL across different domains. Second, we introduce the core concepts and general framework of MTL, the key approaches for sharing knowledge among tasks, and the factors that impact the performance of MTL models. Furthermore, we analyze the structure of various MTL architectures, their advantages, limitations, and how they are classified. Lastly, we provide a comprehensive survey ofresearch efforts that have successfully applied MTL in a wide range of fields.

The remainder of this paper is organized as follows:

Section 2 presents a comprehensive overview of the foundational concepts of MTL, including the factors that influence the effectiveness of any MTL approach. Section 3 provides quantitative insights into the research on Multi-Task Learning focusing on studies indexed in "Scopus". Section 4 explores various MTL architectures with a particular focus on shared knowledge and task-specific components. The categories of multi-task learning architectures are presented in section 5. Section 6 shows the differences between MTL and transfer learning paradigms. Section 7 reviews the applications of MTL across diverse domains such as natural language processing, computer vision, and healthcare. We also present the pros and cons of using MTL in Section 8. Finally, Section 9 concludes the paper and suggests potential future directions for MTL research.

## 2 Foundational concepts of MTL

In this section, we lay the groundwork for understanding multi-task learning by introducing its key concepts and principles. MTL is a machine learning approach where multiple related tasks are learned simultaneously, with the goal of leveraging shared knowledge to improve model performance across tasks. By sharing representations between tasks, MTL aims to enhance the generalization ability of the model, especially in cases where data for individual tasks are scarce or limited [10]. This highlights a more efficient learning paradigm that can minimize memory usage, reduce data consumption, and enhance both training speed and testing performance [15]. The primary objective of MTL is to improve model generalization by utilizing shared representations that can be applied across different tasks [16]. Instead of training separate models for each task, MTL enables a model to learn common features that can benefit multiple tasks.
...
Multi-task learning has gained significant attention in the field of machine learning due to its ability to improve model performance by sharing knowledge across multiple related tasks. As MTL becomes increasingly popular, understand ing its growth trend and application across various domains is essential. This section presents a quantitative analysis of MTL research using “Scopus,” one of the most authoritative databases for scientific publications. The goal is to examine how research on MTL has evolved over the years, from 2020 to 2025, and to explore the key areas of its application.
...
![](images/d5b6742d69f1d0508a6757732c1bb256db54e975a460a640cc518571f6843827.jpg)
...
![](images/013a60e12f785b75ae9a43b3db7257061d0708c015514dbee9325f8d3e7955e4.jpg)
...
| Category | Percentage |
| --- | --- |
| Computer Science | 63.5% |
| Engineering | 15.3% |
| Mathematics | 6.73% |
| Other | 5.97% |
| Medicine & Health Sciences | 3.26% |
| Decision Sciences | 1.53% |
| Neuroscience | 1.03% |
| Psychology | 0.986% |
| Physics and Astronomy | 0.905% |
| Genetics and Molecular Biology | 0.81% |
</details>
...
![](images/6eda040d142cbec62f1dffc6c0a6e1f889f04a4096b1f4412b08990f43faaae8.jpg)
...
| --- | --- |
| Computer Science | 4698 |
| Engineering | 1135 |
| Mathematics | 498 |
| Medicine & Health Sciences | 241 |
| Decision Sciences | 113 |
</details>

Fig. 4 The architecture of hard parameter sharing  
![](images/565719001ab68ce249da8ad014e10f8bef3178c1f602ef7be1637a8d1adfc0cd.jpg)
...
graph TD
  ...
  Task2 --> Task3["Task 3"]
  Task3 --> SharedLayers["Shared Layers"]
  SharedLayers --> Layer1["Layer 1"]
  SharedLayers --> Layer2["Layer 2"]
  SharedLayers --> Layer3["Layer 3"]
```
</details>

## 4.1 Hard parameter sharing

Hard parameter sharing is one of the most widely used strategies in MTL. It involves sharing hidden layers among all task while employing task-specific output layers for predictions [18]. The model weights or parameters are shared between multiple tasks, and each weight is modified to minimize multiple loss functions [19]. This approach promotes knowledge transfer and reduces the risk of overfitting by forcing the model to learn representations that are useful across tasks. They also find applications in diverse fields, from natural language processing to computer vision, etc. Figure 4 illustrates the architecture of hard parameter sharing.

## 4.1.1 Mathematical definition

For the task-specific representation, let $W _ { s }$ represent the shared parameters of the network. The input x is passed through these shared layers to produce a shared representation $\pmb { h } _ { s }$ :

$$
\mathbf {h} _ {\mathrm{s}} = \mathbf {f} _ {\mathrm{s} (\mathbf {x}; \mathbf {W} _ {\mathrm{s}})}, \tag {1}
$$

where $f _ { s }$ is the function representing the shared layers.

For the task-specific heads, for each task $\mathbf { t } \in \{ 1 , 2 , \ldots ,$ T , there is a task-specific set of parameters $\mathbf { w _ { t } }$ . The task specific output $\scriptstyle { y _ { t } }$ is computed as:

$$
\mathbf {y} _ {\mathrm{t}} = \mathbf {f} _ {\mathrm{t} (\mathbf {h} _ {\mathrm{s}}; \mathbf {W} _ {\mathrm{t}})}, \tag {2}
$$

where $f _ { t }$ represents the task-specific layers.

For the loss function, the total loss L for all tasks is a weighted sum of individual task losses:

$$
L = \sum_ {\{t = 1 \}} ^ {T} \lambda_ {t} L _ {t} (y _ {t}, y _ {t} ^ {t r u e}) \tag {3}
$$

where $\lambda _ { t }$ is the weight for task t, $\mathbf { } L _ { t }$ is the loss function for task t, and $y _ { t } ^ { t r u e }$ is the ground truth for task t.

The shared parameters $W _ { s }$ are updated using gradients computed from all tasks, while task-specific parameters $W _ { t }$ are updated based only on their respective task losses.
...
$$
\boldsymbol {L} = \sum_ {\{t = 1 \}} ^ {T} \boldsymbol {L} _ {t} \boldsymbol {\alpha} \boldsymbol {R} (\boldsymbol {W} _ {1}, \boldsymbol {W} _ {2}, \dots , \boldsymbol {W} _ {T}) \tag {4}
$$

where:

$\textstyle \sum _ { \{ t = 1 \} } ^ { T } L _ { t } .$ aggregates the task-specific losses, $\mathbf { } L _ { t }$ be the loss function for taskt,α $\pmb { R } ( \pmb { W } _ { 1 } , \ \pmb { W } _ { 2 } , \ \dots , \ \pmb { W } _ { T } ) \colon \mathbf { A }$ regularization term is added to the loss function to encourage the task-specific parameters $W _ { T }$ to remain similar across tasks (similarity does not mean that all task parameters are identical, they are only allowed to deviate when needed to account for task-specific nuances.), and α is a hyperparameter controlling the strength of regularization.
...
![](images/21b58945b1055f0af998f19b32cb9a5e767fcdeb3c763de4934e83f3e31403c3.jpg)
...
```mermaid
graph LR
  Task1["Task 1"] --> ConstrainedLayers["Constrained Layers"]
  Task2["Task 2"] --> Process1["Process 1"]
  Task3["Task 3"] --> Process2["Process 2"]
  Process1 --> Output1["Output 1"]
  Process2 --> Output2["Output 2"]
  Process1 --> Output3["Output 3"]
  Process2 --> Output4["Output 4"]
  Process3["Process 3"] --> Process1
  Process3 --> Process2
  Process3 --> Process3
  Process4["Process 4"] --> Process2
  Process4 --> Process3
  Process4 --> Process4
  Process5["Process 5"] --> Process2
  Process5 --> Process3
  Process5 --> Process4
  Process6["Process 6"] --> Process2
  Process6 --> Process3
  Process6 --> Process4
  Process7["Process 7"] --> Process2
  Process7 --> Process3
  Process7 --> Process4
  Process8["Process 8"] --> Process2
  Process8 --> Process3
  Process8 --> Process4
  Process9["Process 9"] --> Process2
  Process9 --> Process3
  Process9 --> Process4
  Process10["Process 10"] --> Output1
  Process10 --> Output2
  Process10 --> Output3
  Process10 --> Output4
```
...
<table><tr><td>Aspect</td><td>Hard parameter sharing</td><td>Soft parameter sharing</td></tr><tr><td>Definition</td><td>All tasks share a common set of parameters</td><td>Tasks have their own parameters but share knowledge indirectly</td></tr><tr><td>Task relatedness</td><td>Requires tasks to be highly related</td><td>Suitable for tasks with partial or low relatedness</td></tr><tr><td>Flexibility</td><td>Less flexible; tasks cannot have task-specific features</td><td>More flexible; tasks can learn their specific features</td></tr><tr><td>Computational cost</td><td>Low, as few parameters are used</td><td>Higher, due to maintaining separate parameters for tasks</td></tr><tr><td>Risk of overfitting</td><td>Lower, due to reduced model complexity</td><td>Higher, as each task has more freedom to learn independently</td></tr><tr><td>Adaptability</td><td>Limited adaptability to diverse task requirements</td><td>Better adaptability to unique task requirements</td></tr><tr><td>Implementation</td><td>Simpler and easier to implement</td><td>More complex, requiring mechanisms for task-specific learning</td></tr></table>
...
```mermaid
graph LR
  Input["Input"] --> ConvolutionalLayers["Convolutional Layers"]
  ConvolutionalLayers --> SharedFeatures["Shared Features"]
  SharedFeatures --> MultiTaskEstimation["Multi-task Estimation"]
  SharedFeatures -.-> MainTask["Main Task"]
  SharedFeatures -.-> RelatedTask1["Related task 1"]
  SharedFeatures -.-> ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ...
  Input -.-> ConvolutionalLayers
  ConvolutionalLayers -.-> SharedFeatures
  SharedFeatures ==> MultiTaskEstimation
```
</details>

Fig. 6 A simple architecture of a shared trunk

Fig. 7 Cross-stitch network architecture [14], where a new cross-stitch unit combines two networks into a multi-task network in a way that the tasks supervise how much sharing is needed  
![](images/0e8b05680a70cbd0f891dfa88c4cf186052947d69123bf831056c3e8a743246b.jpg)
...
```mermaid
graph LR
  subgraph Input_Activation_Maps
  A["Task A"] -->|αS| C["αAA αBA"]
  B["Task B"] -->|αD| C
  A -->|αD| D["αBB αAB"]
  B -->|αS| D
  end
  subgraph Cross_stitch_unit
  C --> E["Shared\nTask A"]
  D --> F["Shared\nTask B"]
  end
  E --> G["Output\nActivation Maps"]
...
Reinforcement learning architectures range from simple fully connected, convolutional, and recurrent networks to more complex modular designs for multi-task learning [74]. Joint task training architectures use shared or discrete feature extractors to improve task performance through shared learning signals. Modular policies, such as those developed by [79, 80], decompose networks into reusable submodules, allowing for task-independent and transferable representations. Additionally, auxiliary tasks can guide reinforcement of learning models by enhancing representation learning.

## 5.2 Multimodal architectures

Multimodal architectures extend MTL by integrating data from diverse domains, such as vision and language, to share representations across tasks and media (like images), enhancing generalization. Approaches such as OmniNet [81] use hierarchical or joint-attention mechanisms to efficiently process multiple media. This approach shows improved performance across diverse tasks, especially where datasets are limited, or tasks involve complex interactions between media. Figure 9 shows the OmniNet architecture.

![](images/534eb15c516a2ef191a833d21a1f32846d0b65b98b8f0e83a7467297f3e0e1be.jpg)
...
```mermaid
graph LR
  Input["Input Image"] --> A1["Layer 1"]
  Input --> A2["Layer 2"]
  Input --> A3["Layer 3"]
  A1 --> B1["DECONV"]
  A2 --> B2["DECONV"]
  A3 --> B3["DECONV"]
  A1 --> C1["Multi-Task Predictions"]
  A2 --> C2["Multi-Task Predictions"]
  A3 --> C3["Multi-Task Predictions"]
  B1 --> D1["Multi-Modal Distillation"]
  B2 --> D2["Multi-Modal Distillation"]
  B3 --> D3["Multi-Modal Distillation"]
  C1 --> E1["Multi-Modal Distillation"]
  C2 --> F1["Multi-Modal Distillation"]
  C3 --> G1["Multi-Modal Distillation"]
  D1 --> H1["Output"]
  D2 --> I1["Output"]
  D3 --> J1["Output"]
```
...
![](images/f9c4a2bfd734802f458a054d6baf136283fb3c53af6c22d9e16635235bf395d4.jpg)
...
```mermaid
graph LR
  A["Input Peripheral Image + Video"] --> B["Central Neural Processor"]
  C["Input Peripheral Text"] --> B
  D["Is it cloudy?"] --> C
  B --> E["Temporal Encoder"]
  B --> F["Spatial Cache"]
  E --> G["Temporal Cache"]
  F --> H["Spatio Temporal Decoder"]
  G --> H
  H --> I["Output Captioning"]
  H --> J["Output VQA"]
  H --> K["Output POS"]
  I --> L["An airplane flying through a cloudy sky"]
  J --> M["yes"]
  K --> N["VBZ PRP RB"]
```
</details>
...
Instead of manually designing shared structures, approaches are being developed that allow models to learn both the structure and the weights. This approach allows for the sharing of dynamic parameters tailored to the relationships between tasks, mitigating negative transfer and maximizing positive transfer [74]. These approaches include structure search, branched sharing, modular sharing, and fine-grained sharing. Structure search algorithms, such as reinforcement learning or evolutionary strategies, help design models that can automatically adjust their architecture based on task relationships. Branched sharing gradually partitions shared layers into task-specific layers. Modular sharing enables flexible reuse of recombined components for different tasks. Finegrained sharing applies techniques such as binary masks or pruning techniques to customize parameter sharing, improving task performance. These approaches aim to maximize the benefits of multi-task learning by adapting the sharing strategy to the specific needs of the task, minimizing negative transfer, and enhancing positive transfer.

![](images/fc2e74cd0e95541fd5f8c0903630250968b1c28f89a93a75e2c7b4181ee6eca5.jpg)  
...
In contrast, transfer learning sequentially applies knowledge from a source task to a target task, often through pretrained models, to enhance performance in scenarios where the target task has limited data [22]. Transfer learning refers to exploiting what the model has already been learned in one setting task to improve the learning in another setting [19]. Knowledge transfer happens from the source domain to the target domain [69]. It only focuses on enhancing the performance of the target task. For example, a model previously trained on public text data can be adapted to identify hate speech or offensive language in social media posts or classify sentiment in customer reviews. The knowledge gained from the large-scale language understanding task is transferred to the target task (e.g., hate speech or sentiment analysis), allowing the model to perform better, especially when labeled data for the target task is scarce. Figure 10 shows the paradigms of MTL and transfer learning. While MTL requires datasets for all tasks at the same time, transfer learning typically involves a large dataset for the source task and adapts its knowledge to the target task later.
...
In power systems, MTL enhances tasks such as demand forecasting in power systems [61] and fault detection. For instance, seasonal cycles and energy usage anomalies are better captured via common representations. MTL improves system reliability and operational efficiency in engineering by learning from auxiliary jobs in predictive maintenance, problem categorization, and resource allocation. The paper [93] proposes a multi-task learning approach using a spatiotemporal recurrent imputation network (SRIN) to jointly handle missing performance unit data imputation and shortterm voltage stability assessment. This method effectively combines history-based and feature-based imputation and adapts well to diverse spatiotemporal imputation scenarios. The work [31] proposes a multi-task deep learning framework for fault classification and localization in power systems, achieving high performance with minimal error in fault localization.
...
In autonomous systems, MTL is utilized in tasks such self-driving cars, for tasks including path prediction, lane segmentation, and object identification [62]. For safer and more effective navigation, thejoint learning framework combines temporal and spatial features.
...
Furthermore, MTL in physics allows for shared learning across related problems, like predicting quantum characteristics or solving differential equations for fluid dynamics and heat transfer. To address challenging physics problems, researchers have recently investigated physics-informed neural networks (PINNs) utilizing MTL frameworks. For example [70], M-PINN, a new network structure based on PINNs, improves the representation of multi-physics coupling phenomena by applying soft parameter sharing to fluid–structure interaction (FSI) problems in porous media. This technique showed the potential of MTL to further physics research by outperforming conventional methods in tackling constant pressure production challenges. [40] introduced an MTObased PINN training framework that uses auxiliary tasks to enhance performance via knowledge transfer, achieving improved results in traffic density prediction compared to traditional methods.

In chemistry, [26] used hard parameter sharing and sharing parameters of the LSTM and character embeddings between the named entity recognition (NER) model and the chemical compound paraphrase model. This allows both models to benefit from shared knowledge during training, leading to improved performance in chemical NER. Also, [27] introduces a multi-task deep learning model (MDNN) to predict four flammability-related properties ofchemicals. The model combines Tree-LSTM and feedforward networks to automatically extract molecular features, outperforming traditional methods and eliminating the need for manual feature extraction.
...
Additionally, reduced training time and required resources are a notable benefit of multi-task learning [47, 48]. Because multiple tasks are learned together, a model can often be trained faster than training separate models for each individual task. This can greatly improve efficiency, in large-scale applications.

Table3MTLapplicationsacrossdifferentdomains

<table><tr><td rowspan="2">Ref</td><td rowspan="2">Year</td><td rowspan="2">Field/Domain</td><td rowspan="2">Approach</td><td colspan="2">Parameter Sharing</td><td rowspan="2">Architecture</td><td rowspan="2">Tasks</td><td rowspan="2">Limitations</td></tr><tr><td>Hard</td><td>Soft</td></tr><tr><td>[84]</td><td>2025</td><td>NLP</td><td>Multi-task learning across dialect-specific hate speech tasks</td><td>✓</td><td>-</td><td>Transformer shared encoder with task-specific output heads (e.g., dialect classifiers or binary hate speech classifiers)</td><td>Hate speech detection</td><td>Dialect Imbalance and out-of-vocab problem</td></tr><tr><td>[88]</td><td>2024</td><td>NLP</td><td>Automatic Weight Learning with LLMs</td><td>-</td><td>✓</td><td>LLM-based MTL with auxiliary task construction and weight learning</td><td>Sentiment analysis</td><td>- Data-level uncertainty due to LLM hallucinations- Task-level uncertainty from different model capacities- Performance depends on accurate weight estimation</td></tr><tr><td>[34]</td><td>2021</td><td>NLP</td><td>MTL Approach to Hate Speech Detection using Sentiment Analysis</td><td>✓</td><td>-</td><td>Transformer-based</td><td>Hate Speech Detection, Sentiment Analysis</td><td>Sensitive to implicit hate; struggles with nuanced polarity/emotion cues; mislabeled training data impacts learning; not evaluated beyond Spanish datasets</td></tr><tr><td>[89]</td><td>2022</td><td>Computer vision</td><td>Deep Multi-Task Learning for Malware Image Classification</td><td>✓</td><td>-</td><td>CNN with 11 layers, PReLU activation</td><td>Malware classification across PE, APK, ELF, Mach-O formats</td><td>Limited to image-based features; does not yet include code/API analysis</td></tr><tr><td>[90]</td><td>2024</td><td>Computer vision /Medical Imaging</td><td>Attention-guided Multi-Task Learning</td><td>✓</td><td>✓</td><td>CNN with MTL + Attention + Uncertainty modules</td><td>Skin lesion segmentation and classification</td><td>Limited generalization to other datasets, no domain adaptation, future work needed for task expansion</td></tr></table>

Table3(continued)

<table><tr><td rowspan="2">Ref</td><td rowspan="2">Year</td><td rowspan="2">Field/Domain</td><td rowspan="2">Approach</td><td colspan="2">Parameter Sharing</td><td rowspan="2">Architecture</td><td rowspan="2">Tasks</td><td rowspan="2">Limitations</td></tr><tr><td>Hard</td><td>Soft</td></tr><tr><td>[28]</td><td>2024</td><td>Healthcare</td><td>M3H Multimodal MTL</td><td>✓</td><td>✓</td><td>Attention-based model</td><td>40 diagnoses, 3 operation forecasts, 1 phenotyping (classification, regression, clustering)</td><td>Limited task/pattern coverage (e.g., lack of image segmentation, MS in Computer Science); robustness to data disturbance; lacks integration between omics/wearables; task set design suffers from the curse of dimensionality; numerical instability; future potential in prediction then optimization</td></tr><tr><td>[91]</td><td>2023</td><td>Healthcare</td><td>GenHPF, Multi-task Multi-source Learning</td><td>-</td><td>✓</td><td>Hierarchical Text Encoding</td><td>12 clinical prediction tasks across 3 EHR datasets</td><td>Limited to tabular data and same-language EHRs; excludes some EHR event types due to memory constraints</td></tr><tr><td>[30]</td><td>2023</td><td>Bioinformatics</td><td>UnitedNet, an explainable multi-task deep neural network</td><td>-</td><td>✓</td><td>Encoder-Decoder-Discriminator</td><td>Joint group dentification, Cross-modal prediction</td><td>Requires fine-tuning of the architecture/hyperparameters; randomness in training; and scalability for more tasks that are still being explored</td></tr><tr><td>[93]</td><td>2024</td><td>Bioinformatics/Healthcare/Biomed</td><td>heterogeneous biological network multi-task learning model (HBNMM)</td><td>-</td><td>✓</td><td>Graph Attention + MTL</td><td>ncRNA-disease, ncRNA-drug, drug-disease association prediction</td><td>Sparse associations, imbalance of drug-disease links, limited labeled data, potential for enhancement</td></tr><tr><td>[94]</td><td>2025</td><td>Electrical Engineering/Power Systems</td><td>multi-task learning approach based on spatial-temporal recurrent imputation network (SRIN)</td><td>-</td><td>✓</td><td>SRIN (RNN)</td><td>Missing data imputation, Voltage stability assessment</td><td>Needs further testing across more complex/multiple grids; depends on RNN interpretability</td></tr><tr><td>[23]</td><td>2023</td><td>Medical Imaging</td><td>Cross-Task Attention Network (CTAN)</td><td>-</td><td>✓</td><td>Attention-based MTL</td><td>Segmentation, Diagnosis, Severity Prediction, Dose Planning across 4 datasets (Prostate, OpenKBP, HAM10000, STOIC)</td><td>Limited improvement in segmentation tasks; performance drop in complex tasks like COVID severity classification</td></tr><tr><td>[24]</td><td>2018</td><td>Medicine, Healthcare</td><td>Multi-task Neural Nets for Phenotyping (EHR Data)</td><td>-</td><td>✓</td><td>MTL Neural Networks</td><td>Phenotyping + Auxiliary Tasks</td><td>Random auxiliary task selection may not optimize performance; rule-based ground truth may introduce artifacts; lack of temporal modeling in feature representation; limited phenotype scope</td></tr><tr><td>[25]</td><td>2022</td><td>Healthcare (NLP/RecSys)</td><td>MedRec (Medicine Recommendation System)</td><td>-</td><td>✓</td><td>GCN + Attention + Attribute Fusion</td><td>Graph Embedding, Medicine Recommendation</td><td>Effectiveness depends on graph quality; lacks temporal modeling; limited external validation</td></tr><tr><td>[26]</td><td>2019</td><td>Biomedical NLP, Chemistry</td><td>Multi-task Learning (NER + Paraphrase)</td><td>-</td><td>✓</td><td>Shared Bi-LSTM + ANMT</td><td>NER, Paraphrase Generation</td><td>Dependent on paraphrase quality; limited to chemical domain; requires large paraphrase data</td></tr><tr><td>[27]</td><td>2021</td><td>Chemistry</td><td>Multi-task Deep Neural Network (MDNN) for Predicting Flammability Properties</td><td>-</td><td>✓</td><td>Tree-LSTM encoder + multiple FNN decoders (MDNN)</td><td>Predict Lower Flammability Limit (LFL), Upper Flammability Limit (UFL), Flash Point, and Auto-Ignition Temperature (AIT)</td><td>Requires large, curated datasets; may underperform on small or simple molecules like methane; interpretability limited</td></tr><tr><td>[29]</td><td>2024</td><td>Agriculture</td><td>Hybrid Deep Learning Model</td><td>-</td><td>✓</td><td>MKCNN + Bi-LSTM with Shared Layers</td><td>Regression (yield), Classification (high/low yield)</td><td>Limited data availability; reliance on feature engineering; no temporal dynamics modeled deeply</td></tr><tr><td>[72]</td><td>2023</td><td>Smart Agriculture</td><td>Tensor-based Multi-Task Learning</td><td>-</td><td>✓</td><td>Spatiotemporal Tensor + CP Decomposition</td><td>Predict fertilizer amount and timing across farms and months</td><td>Data collection challenges; complex spatiotemporal modeling assumptions</td></tr><tr><td>[34]</td><td>2021</td><td>NLP</td><td>Multi-Task Learning using BETO</td><td>✓</td><td>✓</td><td>Transformer-based BETO</td><td>Hate Speech Detection, Polarity Classification, Emotion Classification</td><td>Slight degradation on non-HS class, error propagation from auxiliary tasks, label noise in datasets</td></tr><tr><td>[35]</td><td>2023</td><td>NLP</td><td>Deep Multi-Task Learning</td><td>✓</td><td>-</td><td>Bi-LSTM + Task-Specific Perceptrons</td><td>Sentiment Analysis, Sarcasm Detection</td><td>Misclassification of neutral class, sensitivity to sarcastic context, data dependency, preprocessing issues</td></tr><tr><td>[36]</td><td>2022</td><td>NLP/Text Classification</td><td>Multi-task learning with subword-phrase extraction</td><td>✓</td><td>-</td><td>Pretrained NLM + Task-specific FC layers</td><td>Text classification, Subword-phrase recognition</td><td>Dependent on unsupervised subword-phrase extraction quality; no task-weighting scheme implemented</td></tr><tr><td>[37]</td><td>2014</td><td>Computer Vision</td><td>Task-Constrained Learning for Facial Landmark Detection</td><td>✓</td><td>-</td><td>Task-Constrained Deep CNN (TCDCN)</td><td>Facial landmark detection (main), head pose estimation, facial attribute inference, expression recognition, gender, and age estimation</td><td>Tasks have different convergence rates; balancing learning across tasks is challenging</td></tr><tr><td>[38]</td><td>2024</td><td>Intelligent Driving/Computer Vision</td><td>YOLOPv3, Intelligent Driving (Traffic Object Detection, Drivable Area Segmentation, Lane Detection)</td><td>✓</td><td>✓</td><td>YOLOPv3 (based on YOLOv7, encoder-decoder with task-specific heads)</td><td>Object detection, drivable area segmentation, lane detection</td><td>Underutilized high-resolution features and contextual dependencies in previous works; real-time constraints; current model lacks instance segmentation and compression for edge devices</td></tr><tr><td>[39]</td><td>2023</td><td>Computer Vision</td><td>MTL with knowledge distillation</td><td>✓</td><td>✓</td><td>Shared encoder + task-specific heads</td><td>Object detection, Semantic segmentation</td><td>Limited by partial annotations; cannot optimize both tasks simultaneously without distillation</td></tr><tr><td>[40]</td><td>2023</td><td>Traffic Prediction/physics</td><td>Multi-Task Optimization (MTO) for training Physics-Informed Neural Networks (PINNs)</td><td>✓</td><td>✓</td><td>Multiple PINNs with shared architecture and adaptive parameter transfer</td><td>Traffic density prediction (main), traffic speed prediction (auxiliary)</td><td>Complexity in balancing loss components; scalability to more tasks</td></tr><tr><td>[70]</td><td>2024</td><td>Physics</td><td>Physics-Informed Neural Networks (PINNs) utilizing MTL frameworks to fluid-structure interaction problems in porous media</td><td>-</td><td>✓</td><td>MTL-PINN Soft-shared MTL architecture</td><td>Coupled prediction of fluid and structure fields</td><td>Focused on constant pressure production;</td></tr><tr><td>[41]</td><td>2022</td><td>Drug Discovery/biochemistry</td><td>SGNN-EBM (Structured GNN + Energy-Based Model)</td><td>-</td><td>✓</td><td>Soft-sharing via Structured Graph Neural Network (SGNN) and Energy-Based Model (EBM)</td><td>Molecular property prediction (382 tasks)</td><td>Computational complexity; performance sensitive to noise distribution in NCE training</td></tr><tr><td>[42]</td><td>2020</td><td>NLP/Multilingual Neural Machin Translation</td><td>Multi-task Learning with task and data scheduling</td><td>✓</td><td>-</td><td>Transformer-based MNMT</td><td>- Translation (bitext)- MLM (masked language modeling)- DAE (denoising autoencoding)</td><td>Requires careful scheduling Data imbalance across languages Performance drop in high-resource languages</td></tr><tr><td>[43]</td><td>2021</td><td>Medical Imaging/MRI Segmentation</td><td>Deep Multi-Task Learning with weighted losses</td><td>✓</td><td>✓</td><td>Modified V-Net with two decoders + multi-depth fusion module</td><td>- Tumor mask segmentation- Distance transform estimation</td><td>- Weight coefficients set empirically- Not fully optimal- Module suitability (fusion) not validated against other attention types</td></tr><tr><td>[71]</td><td>2022</td><td>Bioinformatics/Drug Discovery</td><td>Multi-task learning + self-supervised pretraining (protein + drug) with dual adaptation mechanism</td><td>✓</td><td>✓</td><td>Protein encoder (Transformer), Drug encoder Graph Convolutional Network (GCN), DTA predictor, MTL framework with dual adaptation</td><td>- DTA (Drug-Target Affinity) prediction- Pretraining (protein/drug)</td><td>- Overfitting on small labeled data- Limited impact from swapping pretrained models</td></tr><tr><td>[73]</td><td>2023</td><td>Healthcare/Recommender Systems</td><td>Bayesian Multi-task Learning with LASSO for ordinal regression</td><td>✓</td><td>✓</td><td>Multivariate Bayesian model with shared priors; Gibbs sampling; Latent variable ordinal probit model</td><td>- Predict satisfaction scores (multicriteria)- Medicine recommendation</td><td>- No overall rating supervision- Lacks personalized utility modeling- Limited patient data (no symptoms/medical history) Meant for clinician use only</td></tr><tr><td>[31]</td><td>2024</td><td>Power Systems/Smart Grids</td><td>CNN-based Multi-task Learning with shared encoder, separate heads</td><td>✓</td><td>-</td><td>- CNN blocks (Conv1D + BN + tanh)- Shared encoder- Two heads: softmax (classification) and linear (regression)</td><td>1. Fault Classification (multi-class)2. Fault Localization (regression)</td><td>- First MTL for power grid faults- Outperforms STL &amp; traditional methods- Noise resilient up to 40dB- Fast inference (&lt; 1 cycle), fewer trainable parameters</td></tr><tr><td>[32]</td><td>2024</td><td>Smart Buildings/Energy</td><td>Multi-task Learning with Mixture-of-Experts + Self-attention (Sparse Transformer) + Multi-gate Feature Fusion</td><td>✓</td><td>✓</td><td>- Multi-gate mixture-of-experts with sparse-constrained transformer- Separate heads for each task- Shared encoder with attention-based fusion layers- Inter-task weighting strategy</td><td>1. Electricity Load Forecasting (regression)2. Air Temperature Forecasting (regression)3. Energy Anomaly Detection (classification)4. Energy Anomaly Prediction (classification)</td><td>- First model to combine anomaly detection + prediction and forecasting in one framework- New public dataset created for heterogeneous building tasks- Load forecasting error reduced by ~60% via MTL- MTL benefits STL via correlated learning- Training difficulty &amp; resource consumption varies across tasks- Larger models ≠ better accuracy- Transformer-based model uses 5x more memory than CNN</td></tr><tr><td>[33]</td><td>2024</td><td>Mental Health/psychology</td><td>MTL for Mental Health (Depression, Stress, Anxiety)</td><td>-</td><td>✓</td><td>Shared base encoder + task-specific heads</td><td>Predict Depression, Anxiety, Stress</td><td>Limited impact of wearable-only features; generalizability beyond college students not explored</td></tr><tr><td>[62]</td><td>2023</td><td>Autonomous Driving</td><td>MTL with MDO (Decision and Optimization)</td><td>✓</td><td>✓</td><td>Shared Backbone + Task-Specific Subnets (UNet, FPN, Transformer)</td><td>Object Detection, Lane Detection, Drivable Area Segmentation, Depth Estimation</td><td>Depth Estimation shows low inter-task correlation (ITC), reducing joint performance; high complexity in optimizing MTL configuration</td></tr></table>

Table3(continued)

<table><tr><td rowspan="2">Ref</td><td rowspan="2">Year</td><td rowspan="2">Field/Domain</td><td rowspan="2">Approach</td><td colspan="2">Parameter Sharing</td><td rowspan="2">Architecture</td><td rowspan="2">Tasks</td><td rowspan="2">Limitations</td></tr><tr><td>Hard</td><td>Soft</td></tr><tr><td>[63]</td><td>2022</td><td>Finance</td><td>CWGAN-GP + Multi-Task Learning</td><td>✓</td><td>-</td><td>CWGAN-GP generator + MTL with shared MLP layers</td><td>1. Credit scoring with imbalanced data2. Synthetic data generation</td><td>High computational cost; long training and tuning time; risk of overfitting with large synthetic sample size; quality and diversity of generated data require improvement;</td></tr><tr><td>[64]</td><td>2021</td><td>Climate science</td><td>Multi-Task Learning + 2D-RNN</td><td>✓</td><td>-</td><td>CNN for feature extraction + 2D-RNN for temporal modeling</td><td>1. Temperature prediction2. Humidity estimation3. Visibility estimation4. Wind speed estimation</td><td>Limited interpretability; no attention mechanism; requires future work on identifying spatially informative regions; computational complexity due to temporal modeling</td></tr><tr><td>[65]</td><td>2019</td><td>Educational Data Mining</td><td>Multi-task learning with multi-layer LSTM and attention mechanism</td><td>-</td><td>✓</td><td>Multi-layer LSTM with shared parameters + Attention</td><td>1. Predict student performance (main task) 2. Predict knowledge point mastery (auxiliary task)</td><td>Focuses only on assignment-related features; does not consider content understanding or external factors; model complexity may increase with task expansion</td></tr><tr><td>[67]</td><td>2020</td><td>Educational Data Mining</td><td>Multi-task MIML (MIML-Circle)</td><td>✓</td><td>✓</td><td>MIML with iterative soft feature fusion</td><td>1. Predict student performance in upcoming courses2. Academic early warning</td><td>Ignores psychological/family factors; assumes prior course info is sufficient; limited dataset availability; generalization across institutions untested</td></tr><tr><td>[66]</td><td>2022</td><td>Robotics/Manipulation</td><td>Multi-task Transformer</td><td>✓</td><td>✓</td><td>Perceiver Transformer (PERACT)</td><td>1. Multi-task 6-DoF robotic manipulation2. Language-conditioned behavior cloning on 18 RLBench &amp; 7 real tasks</td><td>Limited to discrete actions; relies on sampling-based motion planner; hard to generalize to dexterous continuous control (e.g., multi-fingered hands)</td></tr></table>
...
Finally, with the increasing use of MTL systems in highstakes domains, such as healthcare, finance, and autonomous systems, the demand for interpretable and reliable models such as [105, 106] is growing. In these environments, decision transparency, robustness, and accountability are essential, prompting researchers to develop MTL approaches that provide insights into task interactions, representation sharing, and prediction foundations. Enhancing interpretability not only enhances user confidence but also aids in debugging and regulatory compliance. Consequently, interpretable MTL is becoming a critical focus in building reliable AI systems for practical applications.

To conclude, future research in multi-task learning should aim to address current limitations while exploring new opportunities for advancement. Continued innovation will lead to more robust, adaptable, and efficient learning frameworks across a variety of domains.
...
Acknowledgements We would like to express our sincere gratitude to our supervisors for their valuable guidance, insightful comments, and continuous support. Their expertise and encouragement have played a instrumental role in shaping this work. We also extend our appreciation to the Science, Technology & Innovation Funding Authority (STDF) for their financial support, which has been pivotal in enabling this research.
...
7. Marquet, T., and Oswald, E.: A comparison of multi-task learning and single-task learning approaches. in lecture notes in computer science pp. 121–138 (2023) https://doi.org/10.1007/978-3-031- 41181-6\_7  
...
15. Yu, J., Dai, Y., Liu, X., Huang, J., Shen, Y., Zhang, K., Zhou, R., Adhikarla, E., Ye, W., Liu, Y., Kong, Z., Zhang, K., Yin, Y., Namboodiri, V., Davison, B. D., Moore, J. H., and Chen, Y.: Unleashing the power ofmulti-task learning: a comprehensive survey spanning traditional, deep, and pretrained foundation model eras. arXiv (Cornell University) (2024). https://doi.org/10.48550/ arxiv.2404.18961  
...
20. Duong, L., Cohn, T., Bird, S., Cook, P.: Low resource dependency parsing: cross-lingual parameter sharing in a neural network parser (2015). https://doi.org/10.3115/v1/p15-2139  
...
23. Kim, S., Purdie, T.G., and McIntosh, C.: Cross-task attention network: improving multi-task learning for medical imaging applications. In Lecture notes in computer science pp. 119–128 (2023). https://doi.org/10.1007/978-3-031-47401-9\_12  
24. Ding, D.Y., Simpson, C., Pfohl, S., Kale, D.C., Jung, K., Shah, N.H.: The effectiveness of multitask learning for phenotyping with electronic health records data. Biocomputing 24, 18–29 (2018). https://doi.org/10.1142/9789813279827,0003  
...
33. Saylam, B., ˙Incel, Ö.D.: Multitask learning for mental health: depression, anxiety, stress (DAS) using wearables. Diagnostics 14(5), 501 (2024). https://doi.org/10.3390/diagnostics14050501  
...
37. Zhang, Z., Luo, P., Loy, C. C., and Tang, X.: Facial landmark detection by deep multi-task learning. In Lecture notes in computer science pp. 94–108 (2014). https://doi.org/10.1007/978-3- 319-10599-4\_7  
...
67. Ma, Y., Cui, C., Yu, J., Guo, J., Yang, G., Yin, Y.: Multi-task MIML learning for pre-course student performance prediction. Front. Comput. Sci. (2020). https://doi.org/10.1007/s11704-019- 9062-8  
...
82. Luo, S., Chu, V.W., Li, Z., et al.: Multi-task learning by hierarchical Dirichlet mixture model for sparse failure prediction. Int. J. Data Sci. Anal. 12, 15–29 (2021). https://doi.org/10.1007/s 1060-020-00219-z  
...
88. Lai, W., Xie, H., Xu, G., and Li, Q.: Multi-task learning with LLMs for implicit sentiment analysis: data-level and Task-level automatic weight learning. arXiv (Cornell University) (2024). https:// doi.org/10.48550/arxiv.2412.09046  
...
90. Kasukurthi, A., Davuluri, R.L.: AMTLUS: attention-guided multi-task learning with uncertainty estimation in skin lesion segmentation and classification. Multimedia Tools Appl. 83(37), 84885–84909 (2024). https://doi.org/10.1007/s11042- 024-19360-z  
...
94. Xin, D., Ghorbani, B., Garg, A., Firat, O., and Gilmer, J.: Do current multi-task optimization methods in deep learning even help? arXiv (Cornell University) (2022). https://doi.org/10.48550/ arxiv.2209.11379  
95. Peng, C., Yuan, M., and Wang, A.: An exploration of multi-task learning over minBERT (By Stanford CS224N Default Project). https://web.stanford.edu/class/cs224n/final-repo rts/256906365.pdf  
...
100. Bohn, C., Freeman, I., Tercan, H., and Meisen, T.: Task weighting through gradient projection for multitask learning. arXiv (Cornel University) (2024). https://doi.org/10.48550/arxiv.2409.01793  
...
102. Tarunesh, I., Khyalia, S., Kumar, V., Ramakrishnan, G., and Jyothi, P.: Meta-Learning for effective multi-task and multilingua modelling. arXiv (Cornell University) (2021). https://doi.org/10. 48550/arxiv.2101.10368  
...
106. Zelaszczyk, M., Ma´<sup>˙</sup> ndziuk, J.: Interpretable multi-task learning with shared variable embeddings. arXiv (Cornell University) (2024). https://doi.org/10.48550/arxiv.2405.06330
